...
> [!TIP]
> If declare scope message at the beginning of the method in the sources, the Duration value of `Exit Scope` messages will indicate the time in microseconds passed to execute the method as it is highlighted in the screenshot bellow.
...
#### Filtering in Output Log Window

Filter by selecting and double-clicking on a message. This marks a message from the session of the selected scope and displays it in the Output Log Window at the bottom of the application (tab `Scope Analyzes`). Use the following radio buttons to display filtered log messages:
