...
AREG SDK is a interface-centric communication engine. Before creating a service, there should be service interface designed, which defines API of service, and has following sections:
...
The service interfaces can be defined in prototype XML document to use to generate all required base objects to extend and develop either service or service client. An example of the service intface prototype is [Sample.siml](./Sample.siml).
...
Every service interface can have service specific data types. When new data type is defined, it can be used to declare any variables and parameters of methods. New data types are listed in the section `<DataTypeList> ... </DataTypeList>`. All data types are automatically streamable objects.
...
The following example demonstrates how to declare enumeration. All enumerations have `int` type.
```xml
...
_Attributes_ in services are data that clients can subsribe to get update notifications. The attributes are listed in the section `<AttributeList> ... <\AttributeList>`. In this example the system notifies connected clients when the value of `SomeAttr1` is changed, i.e. the system checks whether the new value is equal or not, then updates and sends notification to all subscribed clients:
```xml
...
#### Requests
The requests are called by service clients to run on service side. The requests may have parameters. The requests can be connected with a certain response. If so, the processing request is blocked until service does not reply with response. It is as well possible manually to unblock the request, but then the response must be manually prepared to reply. Mutiple requests can be connected with the same response. The request may have no response at all. 

This example demonstrates definition of request `SomeRequest1` with no parameter connected with a response `SomeResponse1`:
```xml
...
This example demonstrates the defnition of request `SomeRequest2` with multiple parameters connected with response `SomeResponse`:
```xml
...
The responses are sent to the service client as a reply of executed request. When a client calls request to execute on the service, the system automaticaly delivers the response when the service replies. It is as well possible dynamically to subscribes on a response without calling request, so that when a service replies on the request, all subscribed clients as well receive response to react on certain request completion. This example demonstrates the definition of response with and without parameters:
```xml
...
> 💡 The system sends only one response per request when the request is in running state. The multiple responses on one request will have no effect, because the first response will mark request as idle, so that the next replies will have no effect.

#### Broadcasts

Broadcasts are a special methods to send one or more data at the same time. The clinets may dynamically subscribe on _Broadcast_ and each time the service calls broadcast, all subscribed it is delivered to all subscribed clients. The broadcasts are not connected with requestsor responses. This example demonstrates the definition of a boradcast with parameters.
```xml
...
When the service interface prorotype document is designed, developer can use code generator tool to generate bases objects that helps to avoid tedious job and escape mechanical mistakes when parse object. The code generator is located in [tools](./../tools/) folder of areg-sdk. Edit and run `generate.sh` or `generate.bat` file to generate source code.

Before calling code generator, make sure that there is a Java installed on the machine and the `codegen.jar` is included in the `CLASSPATH` evironment variable.
...
Since the codes are generated for both service and the clients, and both of them may be compiled in different projects, we recomend to compile generated codes as a static libraries to link with the executables or shared libraries.

## Modeling and service intiailization

After generating codes, the implementated services must be a part of `Component` object. To initialize services we recomend to create a _model_. The models are created static or dynamic. For an example of dynamic model, see project [14_pubtraffic](./../examples/14_pubtraffic/). This is an example of defining model and start of _Public_ service:
```cpp
