...
| `corpus-toll` | The whole reading corpus -- `AGENTS.md`, the runbook and every page in `docs/agent/` -- at or below 191.1 KB. A page budget bounds one page and says nothing about how many pages there are; this bounds the set, so an addition is paid for by a removal. Raising `CORPUS_CEILING` is a deliberate edit, and the commit that raises it says what the bytes bought. | `FAIL` |
| `duplication` | Under 2% of 12-word runs repeated across three or more pages. | `WARN` |
| `design-review` | A design is reviewable before it is built: `gen_docs.py --spec <design> --review` prints every refusal and every design note, writes nothing, and the runbook names it before `build_project.py --spec`. A note printed only by a generation is one the reader acts on with a `--regenerate`. | `FAIL` |
...
| `name-signatures` | A name the worksheet offers a body carries its declaration, not a bare name: a helper with its parameters, an attribute accessor with its type. A name without a signature is called wrongly and costs a compile. | `FAIL` |
| `name-signatures` | A helper the worksheet offers carries the line its generated header writes above it, not the signature alone. A name whose behaviour is not stated is learnt by opening the file it is declared in: one measured run spent seven requests there. | `FAIL` |
| `order-note` | Every generated body whose behaviour no name gives away is quoted in the worksheet, at the size the largest real project produces. A quoting limit that silently drops one is the same as not quoting it, and the fixture that measures it is generated with the flags the runbook gives. | `FAIL` |
| `order-note` | A run that stalls names what the step sent, what it awaits, and every message an earlier step discarded. A stall that names only the step is a cause the run pays a build-and-run cycle to find. | `FAIL` |
| `step-driver` | A step argument outside the value set its parameter declares is refused before a document is written. A provider that silently ignores a value it does not accept leaves the awaiting step with nothing to end on. | `FAIL` |
...
| Check | Rule | Severity |
|---|---|---|
| `ci-gate` | 13 gates run on every change: documented paths, contract on recipes, recipes build and run, project setup, eval self-check, the corpus check, mutation testing, the observability path, the documented commands run deep, the five-step chain of `build_project.py`, a scenario actually run, the scenario runner's self-test, and a non-Linux runner. A gate under `continue-on-error` reports and never fails a run, which is not a gate. | `FAIL` |
| `fix-bound` | One number for the fix bound, in the four places that state it: the runbook's section 8, the condition each benchmark runner overrides on and the rule text it injects, the `--attempts` default of both runners, and the bound the published baseline records. The runbook's number is what a user's agent is told; the default is what a measured run uses; a run started with no flags at a bound no published run used is not comparable with any of them, and nothing else joins the four. | `FAIL` |
