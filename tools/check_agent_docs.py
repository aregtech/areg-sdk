...
def check_planted_template():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            'setup_project', os.path.join(ROOT, 'tools', 'setup_project.py'))
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
    except Exception as error:                      # noqa: BLE001 - reported, not raised
        return ['tools/setup_project.py: cannot be imported: {}'.format(error)]
    ...
    try:
        ...
        return ['tools/setup_project.py: write_agents failed: {}'.format(error)]
    finally:
