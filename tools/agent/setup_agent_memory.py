...
#
# Scaffolds and checks the local session-knowledge tree under .claude/.
#
# Why this exists
# ---------------
...
# learned it. .gitignore matches *claude*, so nothing this tool creates can be
...
# Usage
# -----
#   python tools/agent/setup_agent_memory.py --init     # create what is missing
#   python tools/agent/setup_agent_memory.py --check    # report, exit 1 if incomplete
#   python tools/agent/setup_agent_memory.py --list     # show the tree and its state
#
# Existing files are never overwritten. No third-party imports, no network, and
# nothing outside .claude/ is written.
# ---------------------------------------------------------------------------
...
# file outside .claude/agent/, because it is the thing a person opens.
BOOTSTRAP = os.path.join('.claude', 'bootstrap.md')
...
1. This file.
2. `.claude/memory/MEMORY.md` -- what previous sessions established.
3. `.claude/lessons/LESSONS.md` -- what previous sessions got wrong.
...
Local only. `.claude/` is gitignored; nothing here is ever committed.
...
def unlisted(kind):
    ...
    """
    index = os.path.join(ROOT, BASE, kind, '%s.md' % kind.upper())
...
def report():
    ...
    for kind in ('memory', 'lessons'):
        ...
        for name in unlisted(kind):
            print('    ^ %s is not named in %s.md' % (name, kind.upper()))
            problems += 1
    return problems
...
def main():
    parser = argparse.ArgumentParser(
        description='Scaffold and check the local session-knowledge tree under '
                    '.claude/. Content is written by hand; this tool only creates '
