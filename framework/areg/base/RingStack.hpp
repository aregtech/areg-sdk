 ...
 *          For more details of capacity flag see areg::OverlapPolicy
...
public:
     ...
     **/
    areg::OverlapPolicy getOverlap() const;
...
private:
     ...
     *              1. areg::Ordering::Equal  -- if 2 lists are equal.
     *              2. areg::Ordering::Bigger -- if `left` stack is bigger than the `right`.
     *              2. areg::Ordering::Smaller-- if `left` stack is smaller than the `right`.
     **/
    areg::Ordering _compareRings(const VALUE* left, uint32_t leftStart, uint32_t leftCapacity, uint32_t leftCount, const VALUE* right, uint32_t rightStart, uint32_t rightCapacity, uint32_t rightCount) const;
...
bool RingStackBase<VALUE>::operator == (const RingStackBase<VALUE>& other) const
{
    ...
    {
        result = _compareRings(mStackList, mHeadPos, mCapacity, mElemCount, other.mStackList, other.mHeadPos, other.mCapacity, other.mElemCount) == areg::Ordering::Equal;
    }
...
template <typename VALUE>
areg::OverlapPolicy RingStackBase<VALUE>::getOverlap() const
{
    return mOnOverlap;
}
...
template <typename VALUE>
areg::Ordering RingStackBase<VALUE>::_compareRings( const VALUE* left, uint32_t leftStart, uint32_t leftCapacity, uint32_t leftCount
