...
bool Mutex::_osLockMutex( unsigned int timeout )
{
    ...
    if ( areg::os::SyncObject0 == SyncLockAndWaitPosix::waitForSingleObject(*syncMutex, timeout) )
    {
...
bool SyncEvent::_osLockEvent(unsigned int timeout)
{
    ...
    return (areg::os::SyncObject0 == SyncLockAndWaitPosix::waitForSingleObject(*syncEvent, timeout));
}
...
bool Semaphore::_osLock( unsigned int timeout )
{
    ...
    return (areg::os::SyncObject0 == SyncLockAndWaitPosix::waitForSingleObject( *syncSemaphore, timeout ));
}
...
{
    return (SyncLockAndWaitPosix::waitForSingleObject( *reinterpret_cast<WaitablePosix *>(mSyncObject), timeout ) == areg::os::SyncObject0);
}
...
int MultiLock::_osLock(unsigned int timeout /* = areg::WAIT_INFINITE */, bool waitForAll /* = false */, bool isAlertable /*= false*/)
{
    ...
    do
    {
        ...
        switch (result)
        {
        case static_cast<int>(areg::os::SyncObjectAll):
        ...
        case static_cast<int>(areg::os::SyncAsyncSignal):
                if (isAlertable)
                {
        ...
        case static_cast<int>(areg::os::SyncObjectTimeout):
            index = MultiLock::LOCK_INDEX_TIMEOUT;
            break;

        default:
            if ( (result >= static_cast<int>(areg::os::SyncObject0)) && (result < mSizeCount) )
            {
                index = result;
