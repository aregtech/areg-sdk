...
AREG_API_IMPL uint32_t areg::crc32Calculate( const uint8_t* data, int32_t size )
{
...
AREG_API_IMPL uint32_t areg::crc32Calculate( const char * strData )
{
    uint32_t result = static_cast<uint32_t>(~0);   // initialize
    if ( strData != nullptr )
    {
...
AREG_API_IMPL uint32_t areg::crc32Calculate( const wchar_t * strData )
{
    uint32_t result = static_cast<uint32_t>(~0);   // initialize
    if ( strData != nullptr )
    {
        ...
        for ( ; *strData != static_cast<wchar_t>('\0'); ++ strData )
        {
            ...
            {
                uint8_t low    = areg::loByte(static_cast<uint16_t>(data));
                uint8_t high   = areg::hiByte(static_cast<uint16_t>(data));
...
AREG_API_IMPL uint32_t areg::crc32Init()
{
    return static_cast<uint32_t>(~0);
}

AREG_API_IMPL uint32_t areg::crc32Start( uint32_t crcInit, const uint8_t* data, int32_t size )
{
...
AREG_API_IMPL uint32_t areg::crc32Start(uint32_t crcInit, const char * data)
{
    uint32_t result = crcInit;
...
AREG_API_IMPL uint32_t areg::crc32Start(uint32_t crcInit, uint8_t uch)
{
    uint32_t result = crcInit;
    const uint32_t* crc32Table = reinterpret_cast<const uint32_t *>(::_crc32LookupTable);  // get converted lookup table
...
AREG_API_IMPL uint32_t areg::crc32Finish( uint32_t crc )
{
    return (~crc);
}
