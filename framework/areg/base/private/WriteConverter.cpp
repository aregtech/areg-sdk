...
{
    const unsigned int size = in_value != nullptr ? (static_cast<unsigned int>(areg::getStringLength<char>(in_value)) + 1u) * sizeof(char) : 0u;
...
{
    const unsigned int size = in_value != nullptr ? (static_cast<unsigned int>(areg::getStringLength<wchar_t>(in_value)) + 1u) * sizeof(wchar_t) : 0u;
    return (size != 0 ? mWriteStream.write(reinterpret_cast<const unsigned char *>(&in_value), size) == size : false);
}

bool WriteConverter::appendString( const char * in_value )
{
    const unsigned int size = in_value != nullptr ? static_cast<unsigned int>(areg::getStringLength<char>(in_value)) * sizeof(char) : 0u;
    return (size != 0 ? mWriteStream.write(reinterpret_cast<const unsigned char *>(in_value), size) == size : false);
}

bool WriteConverter::appendString( const wchar_t * in_value )
{
    const unsigned int size = in_value != nullptr ? static_cast<unsigned int>(areg::getStringLength<wchar_t>(in_value)) * sizeof(wchar_t) : 0u;
    return (size != 0 ? mWriteStream.write(reinterpret_cast<const unsigned char *>(in_value), size) == size : false);
}
