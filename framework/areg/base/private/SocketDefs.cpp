 ...
 **/
AREG_API_IMPL const int areg::MAXIMUM_LISTEN_QUEUE_SIZE{ SOMAXCONN };
...
bool areg::SocketAddress::resolveAddress( const std::string_view & hostName, unsigned short portNr, bool isServer)
{
    bool result{ false };
...
AREG_API_IMPL unsigned int areg::getMaxSendSize( SOCKETHANDLE hSocket )
{
    ASSERT(isSocketHandleValid(hSocket));

    unsigned long maxData{ areg::PACKET_DEFAULT_SIZE };
    return (_osGetOption(hSocket, SOL_SOCKET, SO_SNDBUF, maxData) ? static_cast<unsigned int>(maxData) : areg::PACKET_DEFAULT_SIZE);
}

AREG_API_IMPL unsigned int areg::setMaxSendSize(SOCKETHANDLE hSocket, unsigned int sendSize)
{
    ASSERT(isSocketHandleValid(hSocket));
...
AREG_API_IMPL unsigned int areg::getMaxReceiveSize( SOCKETHANDLE hSocket )
{
    ASSERT(isSocketHandleValid(hSocket));
    unsigned long maxData{ areg::PACKET_DEFAULT_SIZE };
    return (_osGetOption(hSocket, SOL_SOCKET, SO_RCVBUF, maxData) ? static_cast<unsigned int>(maxData) : areg::PACKET_DEFAULT_SIZE);
}

AREG_API_IMPL unsigned int areg::setMaxReceiveSize(SOCKETHANDLE hSocket, unsigned int recvSize)
{
    ASSERT(isSocketHandleValid(hSocket));
...
AREG_API_IMPL SOCKETHANDLE areg::clientSocketConnect(const std::string_view & hostName, unsigned short portNr, areg::SocketAddress * out_socketAddr /*= nullptr*/)
{
...
AREG_API_IMPL SOCKETHANDLE areg::serverSocketConnect(const std::string_view & hostName, unsigned short portNr, SocketAddress * out_socketAddr /*= nullptr */)
{
...
AREG_API_IMPL bool areg::serverListenConnection(SOCKETHANDLE serverSocket, int maxQueueSize /*= areg::MAXIMUM_LISTEN_QUEUE_SIZE*/)
{
    return ( (serverSocket != areg::InvalidSocketHandle) && (areg::RETURNED_OK == listen(serverSocket, maxQueueSize)) );
}

AREG_API_IMPL SOCKETHANDLE areg::serverAcceptConnection(SOCKETHANDLE serverSocket, const SOCKETHANDLE * masterList, int entriesCount, areg::SocketAddress * out_socketAddr /*= nullptr*/)
{
...
AREG_API_IMPL unsigned int areg::pendingRead(SOCKETHANDLE hSocket)
{
...
AREG_API_IMPL int areg::sendData(SOCKETHANDLE hSocket, const unsigned char* dataBuffer, uint32_t dataLength, uint32_t blockMaxSize /*= areg::DEFAULT_SIZE*/)
{
    int result = -1;
    if (isSocketHandleValid(hSocket))
    {
...
AREG_API_IMPL int areg::receiveData(SOCKETHANDLE hSocket, unsigned char* dataBuffer, uint32_t dataLength, uint32_t blockMaxSize )
{
    int result = -1;
