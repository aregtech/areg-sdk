...
//////////////////////////////////////////////////////////////////////////
    ...
    inline int32_t _formatBinary( String & result, DigitType number )
    {
        ...
        char * dst  = buffer;
        DigitType base = static_cast<DigitType>(areg::eRadix::RadixBinary);
        bool isNegative = number < 0;

        number = areg::getAbs<DigitType>(number);
        short idx = 0;
        do
        {
        ...
        *dst    = static_cast<char>(areg::EndOfString);
        ...
        areg::revertString<char>(buffer, count);
        if ( isNegative )
        {
            result = '-';
            ++ count;
     ...
     **/
    template<typename DigitType, int const CharCount = areg::MSG_MIN_BUF_SIZE>
     ...
     * \return  Returns the number of characters in the buffer, not including null-character.
     **/
    template<int const CharCount = areg::MSG_BUF_SIZE>
...
int32_t String::makeInt32( const char * strDigit, areg::eRadix radix /*= areg::RadixDecimal*/, const char ** end /*= nullptr*/ )
{
    char * temp = nullptr;
    int result  = static_cast<int>(areg::isEmpty<char>(strDigit) == false ? strtol( strDigit, &temp, static_cast<int>(radix) ) : 0);
    if (end != nullptr)
    {
...
uint32_t String::makeUInt32( const char * strDigit, areg::eRadix radix /*= areg::RadixDecimal*/, const char ** end /*= nullptr*/ )
{
    char * temp = nullptr;
    unsigned int result = static_cast<unsigned int>(areg::isEmpty<char>(strDigit) == false ? strtoul(strDigit, &temp, static_cast<int>(radix)) : 0);
    if (end != nullptr)
    {
        *end = temp != nullptr ? temp : strDigit;
    }
...
int64_t String::makeInt64( const char * strDigit, areg::eRadix radix /*= areg::RadixDecimal*/, const char ** end /*= nullptr*/ )
{
    char * temp = nullptr;
    int64_t result = static_cast<long long int>(areg::isEmpty<char>(strDigit) == false ? strtoll(strDigit, &temp, static_cast<int>(radix)) : 0);
    if (end != nullptr)
    {
        *end = temp != nullptr ? temp : strDigit;
    }
...
uint64_t String::makeUInt64(const char * strDigit, areg::eRadix radix /*= areg::RadixDecimal*/, const char ** end /*= nullptr*/)
{
    char * temp = nullptr;
    uint64_t result = static_cast<uint64_t>(areg::isEmpty<char>(strDigit) == false ? strtoull(strDigit, &temp, static_cast<int>(radix)) : 0);
    if (end != nullptr)
    {
        *end = temp != nullptr ? temp : strDigit;
    }
...
{
    char * temp = nullptr;
    float result = static_cast<float>(areg::isEmpty<char>(strDigit) == false ? strtof(strDigit, &temp) : 0);
    if (end != nullptr)
    {
        *end = temp != nullptr ? temp : strDigit;
    }
...
{
    char * temp = nullptr;
    double result = static_cast<double>(areg::isEmpty<char>(strDigit) == false ? strtod(strDigit, &temp) : 0);
    if (end != nullptr)
    {
        *end = temp != nullptr ? temp : strDigit;
    }
...
bool String::makeBool( const char * strBoolean, const char ** end /*= nullptr*/ )
{
    bool result = false;
    int lenSkip = 0;
    int lenTrue = static_cast<int>(areg::BOOLEAN_TRUE.length());
    int lenFalse= static_cast<int>(areg::BOOLEAN_FALSE.length());
    if ( areg::compareStrings<char, char>(strBoolean, areg::BOOLEAN_TRUE.data(), lenTrue, false) == areg::eCompare::Equal)
    {
        result = true;
        lenSkip= lenTrue;
    }
    else if ( areg::compareStrings<char, char>(strBoolean, areg::BOOLEAN_FALSE.data(), lenFalse, false) == areg::eCompare::Equal)
    {
        result = false;
        lenSkip= lenFalse;
    }
...
String String::makeString(int32_t number, areg::eRadix radix /*= areg::RadixDecimal */)
{
    String result;

    switch ( radix )
    {
    case areg::eRadix::RadixBinary:
        _formatBinary<int32_t>( result, number );
        break;

    case areg::eRadix::RadixOctal:
        if ( number < 0)
    ...
    case areg::eRadix::RadixHexadecimal:
        if ( number < 0 )
    ...
    case areg::eRadix::RadixDecimal:    // fall through
    case areg::eRadix::RadixAutomatic:  // fall through
    default:
...
String String::makeString(uint32_t number, areg::eRadix radix /*= areg::RadixDecimal */)
{
    String result;
...
String String::makeString(int64_t number, areg::eRadix radix /*= areg::RadixDecimal */)
{
    String result;
...
String String::makeString(uint64_t number, areg::eRadix radix /*= areg::RadixDecimal */)
{
    String result;
...
String & String::formatList(const char * format, va_list argptr)
{
    ...
    if (format != nullptr)
    {
        int count = areg::requiredBufferSize(format, argptr);
