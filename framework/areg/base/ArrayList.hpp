...
void ArrayList< VALUE >::insertAt(uint32_t startAt, const std::vector< VALUE >& newArray)
{
    ...
    if (newArray.empty() == false)
    {
        ...
        {
            limit = static_cast<int>(areg::MAX_CONTAINER_SIZE - (getSize() + static_cast<uint32_t>(newArray.size())));
        }
...
void ArrayList< VALUE >::shift(uint32_t startAt, int  count)
{
    if ((mValueList.size() != 0) && (startAt < mValueList.size()) && (count != 0))
    {
        if (count > 0)
        {
            if (static_cast<uint32_t>(static_cast<int>(getSize()) + count) > areg::MAX_CONTAINER_SIZE)
            {
                count = static_cast<int>(areg::MAX_CONTAINER_SIZE - getSize());
            }
