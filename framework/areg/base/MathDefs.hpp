...
inline void NEMath::setBits( LargeInteger &num, uint32_t hi, uint32_t lo )
{
    num.hiBits = hi;
    num.loBits = lo;
}

inline uint32_t NEMath::getHighBits( const LargeInteger &num )
{
    return (num.hiBits);
}

inline uint32_t NEMath::getLowBits( const LargeInteger &num )
{
    return (num.loBits);
}
