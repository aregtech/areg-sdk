...
private:
    ...
    public:
         ...
         **/
        Listener( unsigned int msgId, const SequenceNumber & seqNr );
...
protected:
     ...
     **/
    static constexpr unsigned int MINIMAL_DELAY_TIME_MS { areg::WAIT_5_MILLISECONDS };
...
public:
     ...
     **/
    static RemoteResponseEvent * createRequestFailureEvent(const ProxyAddress & target, unsigned int msgId, areg::ResultType errCode, const SequenceNumber & seqNr);
...
protected:
     ...
     **/
    ProxyBase( const String & roleName, const areg::SInterfaceData & serviceIfData, DispatcherThread * ownerThread = nullptr );
...
/************************************************************************/
     ...
     **/
    virtual ServiceRequestEvent * createNotificationRequestEvent( unsigned int msgId, areg::eRequestType reqType ) = 0;
     ...
     **/
    virtual RemoteResponseEvent * createRemoteRequestFailedEvent( const ProxyAddress & addrProxy, unsigned int msgId, areg::ResultType reason, const SequenceNumber & seqNr ) const;
...
/************************************************************************/
     ...
     **/
    virtual void serviceConnectionUpdated( const StubAddress & server, const Channel & channel, areg::eServiceConnection status ) override;
...
/************************************************************************/
     ...
     **/
    virtual void sendNotificationEvent( unsigned int msgId, areg::ResultType resType, const SequenceNumber & seqNr, NotificationConsumer * caller );
...
protected:
     ...
     **/
    inline void setState( unsigned int msgId, areg::eDataStateType newState );

    /**
     * \brief   Sets the connection status of the proxy
     **/
    inline void setConnectionStatus(areg::eServiceConnection status);
     ...
     *                          If areg::SEQUENCE_NUMBER_NOTIFY,
     *                          all notification listeners assigned for specified
     *                          message ID will get notification.
     **/
    void notifyListeners( unsigned int respId, areg::ResultType result, const SequenceNumber & seqNrToSearch );
     ...
     *                      See details in areg::eRequestType
     **/
    void sendNotificationRequestEvent( unsigned int msgId, areg::eRequestType reqType );
...
inline void ProxyBase::setConnectionStatus(areg::eServiceConnection status)
{
    mConnectionStatus = status;
    mIsConnected = areg::isServiceConnected(status);
}

inline areg::eServiceConnection ProxyBase::getConnectionStatus() const
{
...
inline void ProxyBase::startNotification( unsigned int msgId )
{
    if (isConnected())
    {
        sendNotificationRequestEvent( msgId, areg::eRequestType::StartNotify );
    }
}
...
inline void ProxyBase::stopNotification( unsigned int msgId )
{
    if (isConnected()) 
    {
        sendNotificationRequestEvent( msgId, areg::eRequestType::StopNotify );
    }
}
...
inline void ProxyBase::stopAllServiceNotifications()
{
    if (isConnected()) 
    {
        sendNotificationRequestEvent( static_cast<unsigned int>(areg::eFuncIdRange::EmptyFunctionId), areg::eRequestType::RemoveAllNotify );
    }
}
...
inline void ProxyBase::setState( unsigned int msgId, areg::eDataStateType newState )
{
