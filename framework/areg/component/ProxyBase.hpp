...
protected:
    ...
    public:
         ...
         **/
        inline void setEventDelay(uint32_t msDelay)
        {
         ...
         **/
        inline uint32_t getEventDalay() const
        {
...
public:
     ...
     *          for existing proxy object, instantiated int32_t current thread.
     ...
     **/
    static RemoteResponseEvent * createRequestFailureEvent(const ProxyAddress & target, uint32_t msgId, NEService::ResultType errCode, const SequenceNumber & seqNr);
...
public:
     ...
     **/
    inline bool hasAnyListener(uint32_t msgId) const;
     ...
     *          for specified message ID.
     **/
    inline bool hasNotificationListener(uint32_t msgId) const;
     ...
     **/
    inline uint32_t getListenerCount() const;
...
/************************************************************************/
     ...
     **/
    virtual ServiceRequestEvent * createNotificationRequestEvent( uint32_t msgId, NEService::RequestType reqType ) = 0;
     ...
     **/
    virtual RemoteResponseEvent * createRemoteRequestFailedEvent( const ProxyAddress & addrProxy, uint32_t msgId, NEService::ResultType reason, const SequenceNumber & seqNr ) const;
...
/************************************************************************/
     ...
     **/
    virtual void sendNotificationEvent( uint32_t msgId, NEService::ResultType resType, const SequenceNumber & seqNr, NotificationConsumer * caller );
...
protected:
     ...
     **/
    inline void startNotification( uint32_t msgId );
     ...
     **/
    inline void stopNotification( uint32_t msgId );
     ...
     **/
    inline void stopNotifications( const uint32_t notifyIds[], int32_t count );
     ...
     **/
    inline void setState( uint32_t msgId, NEService::DataState newState );
     ...
     **/
    void notifyListeners( uint32_t respId, NEService::ResultType result, const SequenceNumber & seqNrToSearch );
     ...
     **/
    uint32_t prepareListeners( ProxyBase::ProxyListenerList & out_listenerList, uint32_t msgId, const SequenceNumber & seqNrToSearch );
     ...
     **/
    void sendNotificationRequestEvent( uint32_t msgId, NEService::RequestType reqType );
...
inline void ProxyBase::stopNotification( uint32_t msgId )
{
    if (isConnected()) 
    {
...
inline void ProxyBase::stopAllServiceNotifications()
{
    if (isConnected()) 
    {
        sendNotificationRequestEvent( static_cast<uint32_t>(NEService::FuncIdRange::EmptyFunctionId), NEService::RequestType::RemoveAllNotify );
    }
}

inline void ProxyBase::stopNotifications( const uint32_t notifyIds[], int32_t count )
{
...
inline void ProxyBase::setState( uint32_t msgId, NEService::DataState newState )
{
...
inline uint32_t ProxyBase::getListenerCount() const
{
