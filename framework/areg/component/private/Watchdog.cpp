...
Watchdog::Watchdog(ComponentThread& thread, uint32_t msTimeout /*= areg::common::WATCHDOG_IGNORE*/)
...
Watchdog::Watchdog(WorkerThread& thread, uint32_t msTimeout /*= areg::common::WATCHDOG_IGNORE*/)
    : TimerBase         ( TimerBase::eTimerType::TimerTypeWatchdog, thread.getName(), msTimeout, TimerBase::ONE_TIME)
    , mGuardId          ( _generateId() )
    , mSequence         ( 0u )
...
{
    if (mTimeoutInMs != areg::common::WATCHDOG_IGNORE)
    {
        Lock lock(mLock);
