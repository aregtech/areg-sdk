...
Thread::eCompletionStatus ComponentThread::shutdownThread( unsigned int waitForStopMs /*= areg::DO_NOT_WAIT*/ )
{
    ListComponent::LISTPOS pos = mListComponent.firstPosition( );
    while ( mListComponent.isValidPosition( pos ) )
    {
...
bool ComponentThread::completionWait( unsigned int waitForCompleteMs /*= areg::WAIT_INFINITE */ )
{
    ListComponent::LISTPOS pos = mListComponent.firstPosition();
    while ( mListComponent.isValidPosition(pos) )
    {
