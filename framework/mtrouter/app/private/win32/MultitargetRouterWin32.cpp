...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
    ...
    router.parseOptions(static_cast<int>(argc), argvTemp, aregext::ServiceOptionSetup, std::size(aregext::ServiceOptionSetup));
...
VOID WINAPI _win32ServiceMain( DWORD argc, LPTSTR * argv )
{
    ...
    {
        MultitargetRouter& router = MultitargetRouter::getInstance();
        router.setState(aregext::eSystemServiceState::ServiceStarting);
        char** argvTemp = aregext::convertArguments<TCHAR>(argv, static_cast<int>(argc));
        router.serviceMain(aregext::eServiceOption::CMD_Service, argvTemp != nullptr ? argvTemp[0] : nullptr);
        aregext::deleteArguments(argvTemp, static_cast<int>(argc));
        router.setState(aregext::eSystemServiceState::ServiceStopped);
    }
