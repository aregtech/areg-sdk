...
void ObserverMessageProcessor::notifyServiceConnection(const RemoteMessage& msgReceived)
{
    ...
    RemoteMessage msgLog = areg::createLogMessage(log, areg::eLogDataType::LogDataLocal, cookie);
    notifyLogMessage(msgLog);
}

void ObserverMessageProcessor::notifyConnectedClients(const RemoteMessage& msgReceived)
{
    areg::eRemoteConnection remConnect{ areg::eRemoteConnection::RemoteDisconnected };
    ...
    do
    {
        ...
        if (remConnect == areg::eRemoteConnection::RemoteConnected)
        {
...
void ObserverMessageProcessor::notifyLogRegisterScopes(const RemoteMessage& msgReceived)
{
    ...
    do
    {
        ...
        RemoteMessage msgLog = areg::createLogMessage(log, areg::eLogDataType::LogDataLocal, areg::COOKIE_LOGGER);
        notifyLogMessage(msgLog);
...
void ObserverMessageProcessor::notifyLogMessage(const RemoteMessage& msgReceived)
{
    ...
    {
        Lock lock(mLoggerClient.mLock);
        const areg::sLogMessage* msgRemote = reinterpret_cast<const areg::sLogMessage*>(msgReceived.getBuffer());
        ASSERT(msgRemote != nullptr);
        const_cast<areg::sLogMessage*>(msgRemote)->logReceived = static_cast<TIME64>(now);
...
{
    ArrayList< areg::sServiceConnectedInstance > listConnected;
    ...
    do
    {
        ...
        if (LogObserverBase::_theLogObserver != nullptr)
        {
            ...
            {
                const areg::sServiceConnectedInstance& client{ listConnected[static_cast<uint32_t>(i)] };
        ...
        else
        {
            ...
            for (int i = 0; i < size; ++i)
            {
                ...
                if (listInstances != nullptr)
                {
                    ...
                    areg::memCopy(inst.liName    , LENGTH_NAME    , client.ciInstance.c_str(), static_cast<int>(client.ciInstance.length()) + 1);
                    areg::memCopy(inst.liLocation, LENGTH_LOCATION, client.ciLocation.c_str(), static_cast<int>(client.ciLocation.length()) + 1);
                }
            }
...
void ObserverMessageProcessor::_clientsDisconnected(const RemoteMessage& msgReceived)
{
    ...
    ArrayList< areg::sServiceConnectedInstance > listDisconnected;
    ...
    do
    {
        ...
        if (size > 0)
        {
            ...
            for (int i = 0; i < size; ++i)
            {
                ...
                {
                    const areg::sServiceConnectedInstance& instance = mLoggerClient.mInstances.getAt(client);
...
inline void ObserverMessageProcessor::_initLocalLogMessage(areg::sLogMessage& log, ITEM_ID cookie, TIME64 timestamp /*= 0*/) const
{
