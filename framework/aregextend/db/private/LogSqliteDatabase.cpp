...
inline void LogSqliteDatabase::_initialize()
{
    ...
    String::formatString(sql, SQL_LEN, _fmtLog.data()
                        ...
                        , static_cast<uint32_t>(areg::eLogMessageType::LogMessageText)
                        , static_cast<uint32_t>(areg::eLogPriority::PrioIgnore)
...
{
    constexpr uint32_t _logSize{ static_cast<uint32_t>(sizeof(areg::sLogMessage)) };
    buf.reserve(_logSize, false);
    buf.setSizeUsed(_logSize);
    buf.moveToBegin();
    areg::sLogMessage* log = reinterpret_cast<areg::sLogMessage*>(buf.getBuffer());

    ASSERT(log != nullptr);
    log->logDataType = areg::eLogDataType::LogDataRemote;
    log->logSource = areg::COOKIE_ANY;
    log->logTarget = areg::COOKIE_ANY;

    log->logMsgType     = static_cast<areg::eLogMessageType>(stmt.getUint32(0));
    log->logMessagePrio = static_cast<areg::eLogPriority>(stmt.getUint32(1));
    ...
    areg::copyStringFast(log->logMessage, msg.getString(), msg.getLength());
    areg::copyStringFast(log->logThread, thread.getString(), thread.getLength());
    areg::copyStringFast(log->logModule, module.getString(), module.getLength());
}

inline void LogSqliteDatabase::_copyLogInstances(SqliteStatement& stmt, areg::sServiceConnectedInstance& inst)
{
    inst.ciSource   = static_cast<areg::eMessageSource>(  stmt.getUint32(0));
    inst.ciBitness  = static_cast<areg::eInstanceBitness>(stmt.getUint32(1));
...
inline void LogSqliteDatabase::_copyLogScopes(SqliteStatement& stmt, areg::sScopeInfo& scope)
{
...
bool LogSqliteDatabase::isOperable() const
{
...
bool LogSqliteDatabase::areTablesInitialized() const
{
    return mIsInitialized;
}

bool LogSqliteDatabase::logMessage(const areg::sLogMessage& message)
{
    Lock lock(mLock);
...
bool LogSqliteDatabase::logInstanceConnected(const areg::sServiceConnectedInstance& instance, const DateTime& timestamp)
{
    Lock lock(mLock);
    Process& proc    { Process::getInstance() };
    String   module  { proc.getAppName() };
...
bool LogSqliteDatabase::logScopeActivate(const areg::sScopeInfo & scope, const ITEM_ID& cookie, const DateTime& timestamp)
{
...
std::vector<String> LogSqliteDatabase::getPriorityNames()
{
    std::vector<String> result{
        { 
          areg::logPrioToString(areg::eLogPriority::PrioAny)
        , areg::logPrioToString(areg::eLogPriority::PrioScope)
        , areg::logPrioToString(areg::eLogPriority::PrioDebug)
        , areg::logPrioToString(areg::eLogPriority::PrioInfo)
        , areg::logPrioToString(areg::eLogPriority::PrioWarning)
        , areg::logPrioToString(areg::eLogPriority::PrioError)
        , areg::logPrioToString(areg::eLogPriority::PrioFatal)
        }
    };
    return result;
}
...
std::vector<areg::sServiceConnectedInstance> LogSqliteDatabase::getLogInstanceInfos()
{
    std::vector<areg::sServiceConnectedInstance> result;
    getLogInstanceInfos(result);
    return result;
}

void LogSqliteDatabase::getLogInstanceInfos(std::vector<areg::sServiceConnectedInstance>& infos)
{
    Lock lock(mLock);
    infos.clear();
    ...
    if (stmt.isValid())
    {
        while (stmt.next() == SqliteStatement::eQueryResult::HasMore)
        {
            areg::sServiceConnectedInstance inst;
...
std::vector<areg::sScopeInfo> LogSqliteDatabase::getLogInstScopes(ITEM_ID instId)
{
    std::vector<areg::sScopeInfo> result;
    getLogInstScopes(result, instId);
    return result;
}

void LogSqliteDatabase::getLogInstScopes(std::vector<areg::sScopeInfo>& scopes, ITEM_ID instId)
{
    Lock lock(mLock);
    scopes.clear();
...
int LogSqliteDatabase::getLogInstScopes(std::vector<areg::sScopeInfo>& scopes, SqliteStatement& stmt, int maxEntries /*= -1*/)
{
    int result{ 0 };
    if (stmt.isValid())
    {
...
int LogSqliteDatabase::getLogMessages(std::vector<SharedBuffer>& logs, SqliteStatement& stmt, int maxEntries /*= -1*/)
{
    int result{ 0 };
    if (stmt.isValid())
    {
...
int LogSqliteDatabase::fillLogInstances(std::vector<areg::sServiceConnectedInstance>& infos, SqliteStatement& stmt)
{
    int result{ 0 };
    if ((static_cast<uint32_t>(infos.size()) != 0) && stmt.isValid())
    {
        while (stmt.next() == SqliteStatement::eQueryResult::HasMore)
        {
            ...
            areg::sServiceConnectedInstance& inst{ infos[result] };
            _copyLogInstances(stmt, inst);
            ++result;
        }
    }
...
int LogSqliteDatabase::fillInstScopes(std::vector<areg::sScopeInfo>& scopes, SqliteStatement& stmt, uint32_t startAt, int maxEntries)
{
    int result{ 0 };
    if ((static_cast<uint32_t>(scopes.size()) > startAt) && stmt.isValid())
    {
        while (stmt.next() == SqliteStatement::eQueryResult::HasMore)
        {
            ...
            areg::sScopeInfo& scope{ scopes[startAt + static_cast<uint32_t>(result)] };
            _copyLogScopes(stmt, scope);
            ++result;
            if ((maxEntries > 0) && (result >= maxEntries))
                break;
...
int LogSqliteDatabase::fillLogMessages(std::vector<SharedBuffer>& logs, SqliteStatement& stmt, uint32_t startAt, int maxEntries)
{
    int result{ 0 };
