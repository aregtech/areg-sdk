#ifndef AREG_IPC_PRIVATE_CESERVERCONNECTION_HPP
#define AREG_IPC_PRIVATE_CESERVERCONNECTION_HPP

/************************************************************************
 * \file        areg/src/ipc/private/CEServerConnection.hpp
...
#include "areg/src/base/CESocketServer.hpp"
#include "areg/src/base/CESocketAccepted.hpp"
 ...
 **/
class AREG_API CEServerConnection   : private   CESocketConnectionBase
{
    /**
     * \brief   The container of accepted socket objects where the keys are socket handle.
     **/
    typedef TEHashMapImpl<const SOCKETHANDLE, const CESocketAccepted &>                                                     MapSocketToObjectImpl;
    typedef TEHashMap<SOCKETHANDLE, CESocketAccepted, const SOCKETHANDLE, const CESocketAccepted &, MapSocketToObjectImpl>  MapSocketToObject;
    /**
...
public:
     ...
     **/
    CEServerConnection( const char * hostName, unsigned short portNr );
     ...
     **/
    CEServerConnection( const NESocket::CEInterlockedValue & serverAddress );

    /**
     * \brief   Destructor.
     **/
    virtual ~CEServerConnection( void );
...
public:
     ...
     **/
    ITEM_ID GetCookie( const CESocketAccepted & clientSocket ) const;
     ...
     **/
    CESocketAccepted GetClientByCookie( ITEM_ID clientCookie ) const;
     ...
     *          the returned object is valid accepted client object. Otherwise, the object is invalid.
     **/
    CESocketAccepted GetClientByHandle( SOCKETHANDLE clientSocket ) const;
...
public:
     ...
     **/
    SOCKETHANDLE WaitForConnectionEvent(NESocket::CEInterlockedValue & out_addrNewAccepted);
     ...
     **/
    bool AcceptConnection( CESocketAccepted & clientConnection );
     ...
     **/
    void RejectConnection( CESocketAccepted & clientConnection );

    /**
     * \brief   Call to close client connection.
     **/
    void CloseConnection( CESocketAccepted & clientConnection );
     ...
     **/
    inline int SendMessage( const CERemoteMessage & in_message, const CESocketAccepted & clientSocket ) const;
     ...
     **/
    inline int ReceiveMessage( CERemoteMessage & out_message, const CESocketAccepted & clientSocket ) const;
     ...
     *          Returns zero, if checksum in Remote Buffer was not validated or Remote Buffer object is empty.
     **/
    inline int SendMessage( const CERemoteMessage & in_message, ITEM_ID clientCookie ) const;
     ...
     *          Returns zero, if checksum in Remote Buffer was not validated or data in Remote Buffer object is empty.
     **/
    inline int ReceiveMessage( CERemoteMessage & out_message, ITEM_ID clientCookie ) const;

    /**
     * \brief   Sets socket in read-only more, i.e. no send message is possible anymore.
     * \return  Returns true if operation succeeds.
     **/
    inline bool DisableSend( const CESocketAccepted & clientConnection );

    /**
     * \brief   Sets socket in write-only more, i.e. no receive message is possible anymore.
     * \return  Returns true if operation succeeds.
     **/
    inline bool DisableReceive( const CESocketAccepted & clientConnection );
...
//////////////////////////////////////////////////////////////////////////
private:
    CEServerConnection( const CEServerConnection & );
    const CEServerConnection & operator = ( const CEServerConnection & );
};

//////////////////////////////////////////////////////////////////////////
// CEServerConnection class inline functions
//////////////////////////////////////////////////////////////////////////

inline bool CEServerConnection::SetAddress(const char * hostName, unsigned short portNr)
{   CELock lock(mLock); return mServerSocket.SetAddress(hostName, portNr, true);                }

inline void CEServerConnection::SetAddress( const NESocket::CEInterlockedValue & newAddress )
{   CELock lock(mLock); mServerSocket.SetAddress(newAddress);                                   }

inline const NESocket::CEInterlockedValue & CEServerConnection::GetAddress( void ) const
{   CELock lock(mLock); return mServerSocket.GetAddress();                                      }

inline bool CEServerConnection::IsValid( void ) const
{   CELock lock(mLock); return mServerSocket.IsValid();                                         }

inline SOCKETHANDLE CEServerConnection::GetSocketHandle( void ) const
{   CELock lock(mLock); return mServerSocket.GetHandle();                                       }

inline bool CEServerConnection::IsConnectionAccepted( SOCKETHANDLE connection ) const
{   CELock lock(mLock); return (mAcceptedConnections.Find(connection) != NULL);                 }

inline int CEServerConnection::SendMessage(const CERemoteMessage & in_message, const CESocketAccepted & clientSocket) const
{   return CESocketConnectionBase::SendMessage( in_message, clientSocket);                      }

inline int CEServerConnection::SendMessage(const CERemoteMessage & in_message, ITEM_ID clientCookie) const
{   return CESocketConnectionBase::SendMessage( in_message, GetClientByCookie(clientCookie) );  }

inline int CEServerConnection::ReceiveMessage(CERemoteMessage & out_message, const CESocketAccepted & clientSocket) const
{   return CESocketConnectionBase::ReceiveMessage(out_message, clientSocket);                   }

inline int CEServerConnection::ReceiveMessage(CERemoteMessage & out_message, ITEM_ID clientCookie) const
{   return CESocketConnectionBase::ReceiveMessage( out_message,GetClientByCookie(clientCookie));}

inline bool CEServerConnection::DisableSend( const CESocketAccepted & clientConnection )
{   return clientConnection.DisableSend();                                                      }

inline bool CEServerConnection::DisableReceive( const CESocketAccepted & clientConnection )
{   return clientConnection.DisableReceive();                                                   }

#endif  // AREG_IPC_PRIVATE_CESERVERCONNECTION_HPP
