...
unsigned int File::read( IEByteBuffer & buffer ) const
{
    ...
    if ( isOpened() && canRead() )
    {
        ...
        if ( (::read(file->fd, &sizeReserve, sizeof(int)) == sizeof(int)) && (sizeReserve > 0) )
        {
            ...
            {
                OUTPUT_ERR("Either was not able to reserve [ %d ] bytes of space, or failed read file [ %s ], error code [ %p ].", sizeReserve, mFileName.getString(), errno);
            }
        }
        else
        {
            OUTPUT_ERR("Unable to read the size of byte-buffer, the file [ %s ], error code [ %p ]", mFileName.getString(), errno);
        }
    }
    else
    {
...
unsigned int File::write( const IEByteBuffer & buffer )
{
    ...
    if ( isOpened() && canWrite() )
    {
        ...
        {
            OUTPUT_ERR("Failed to write [ %d ] bytes of data in file [ %s ], error code [ %p ]", sizeUsed + sizeof(int), mFileName.getString(), errno);
        }
    }
    else
    {
        OUTPUT_ERR("Either files [ %s ] is not opened or it is not opened to write file, error code [ %p ]", mFileName.getString(), errno);
    }
