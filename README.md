...
**AREG IoT SDK** (or **AREG SDK**) consists of a framework and tools that simplify multitasking and service enabled applications development for embedded and/or high-end machines. 

AREG SDK can be used to develop a single embedded application (process), as well as to develop complex multi-process embedded or desktop applications running on one or several machines. AREG SDK is event-driven, asynchronous, and fault tolerant. It combines features of client-server and publish-subscribe communication model. It automatically provides modularity, forms a mesh of distributed services and keeps the service discovery transparent. So that, in the complex multi-processing projects the architects may easily distribute the software components in various processes running on one or different machines, and this remains transparent for software developers as they would write application codes for a single process or even a single thread.

The project AREG Framework has been started as an asynchronous multithreading framework to control a machine in a digital twin project, and it has grown as a software development platform that supports intelligent embedded- and mist-computing. Our agenda is to make AREG SDK powerful and flexible, but an easy and simple to use development tool that provides mist-computing at the edge of the IoT network, runs on various operating systems and supports various communication channels. We focus on technology to reduce software development time and help software developers to focus on application business logic.
