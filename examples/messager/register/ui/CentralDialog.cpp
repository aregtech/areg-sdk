...
bool CentralDialog::StartConnection( const String & ipAddress, unsigned short portNr )
{
    ...
    if ( dlg != NULL )
    {
        if ( Application::startMessageRouting(ipAddress, portNr) )
        {
            ...
            ComponentLoader::setComponentData( NECommonSettings::COMP_NAME_CENTRAL_SERVER, compData );
            if ( Application::loadModel( NECommonSettings::MODEL_NAME_CENTRAL_SERVER ) )
            {
...
BEGIN_MESSAGE_MAP(CentralDialog, CPropertySheet)
    ...
    ON_MESSAGE( NECentralApp::CmdServiceConnection     , &CentralDialog::OnCmdServiceConnection )
END_MESSAGE_MAP()
