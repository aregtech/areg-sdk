// edge/PageConnections.cpp : implementation file
//
...
#include "edge/services/DirectConnectionService.hpp"
...
void PageConnections::OnServiceNetwork( bool isConnected, DispatcherThread * ownerThread )
{
    if ( isConnected && (ownerThread != NULL) )
    {
        ...
        {
            mClientConnections = DEBUG_NEW ConnectionList( NECommonSettings::COMP_NAME_CENTRAL_SERVER, *ownerThread, mConnectionHandler );
        }
    }
}
...
void PageConnections::OnClientConnection( bool isConnected, DispatcherThread *dispThread )
{
    if ( isConnected )
    {
        if ( mClientConnections == NULL )
        {
            ...
            mClientConnections = DEBUG_NEW ConnectionList( NECommonSettings::COMP_NAME_CENTRAL_SERVER, *dispThread, mConnectionHandler );
        }
    }
    else
    {
