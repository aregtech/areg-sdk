...
CEDirectMessagerRequestEvent::CEDirectMessagerRequestEvent( const CEProxyAddress & fromSource, const CEStubAddress & toTarget, unsigned int reqId )
    : CERemoteRequestEvent( fromSource, toTarget, reqId )
{
    ; // do nothing
}

CEDirectMessagerRequestEvent::CEDirectMessagerRequestEvent( const CEEventDataStream & args, const CEProxyAddress & fromSource, const CEStubAddress & toTarget, unsigned int reqId )
    : CERemoteRequestEvent( args, fromSource, toTarget, reqId, NEDirectMessager::GetString( static_cast<NEDirectMessager::eMessageIDs>(reqId) ) )
{
    ; // do nothing
}
...
CEDirectMessagerNotifyRequestEvent::CEDirectMessagerNotifyRequestEvent( const CEProxyAddress & fromProxy, const CEStubAddress & toStub, unsigned int msgId, NEService::eRequestType reqType )
...
CEDirectMessagerResponseEvent::CEDirectMessagerResponseEvent( const CEProxyAddress & proxyTarget, NEService::eResultType result, unsigned int responseId, unsigned int seqNr /* = NEService::SEQUENCE_NUMBER_NOTIFY */ )
    : CERemoteResponseEvent( proxyTarget, result, responseId, seqNr )
{
    ; // do nothing
}

CEDirectMessagerResponseEvent::CEDirectMessagerResponseEvent( const CEEventDataStream & args, const CEProxyAddress & proxyTarget, NEService::eResultType result, unsigned int responseId, unsigned int seqNr /* = NEService::SEQUENCE_NUMBER_NOTIFY */ )
    : CERemoteResponseEvent( args, proxyTarget, result, responseId, seqNr, NEDirectMessager::GetString(static_cast<NEDirectMessager::eMessageIDs>(responseId)) )
{
    ; // do nothing
}
