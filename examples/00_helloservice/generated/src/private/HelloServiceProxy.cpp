...
RemoteResponseEvent * HelloServiceProxy::createRemoteRequestFailedEvent(const ProxyAddress & addrProxy, unsigned int msgId, NEService::eResultType reason, const SequenceNumber &  seqNr) const
{
...
{
    ProxyBase::ServiceAvailableEvent* event = static_cast<ProxyBase::ServiceAvailableEvent *>( DEBUG_NEW HelloServiceProxy::HelloServiceServiceAvailableEvent(consumer) );
    if (event != nullptr)
    {
...
const SequenceNumber &  HelloServiceProxy::requestHelloService( IENotificationEventConsumer & caller, const String & client )
{
