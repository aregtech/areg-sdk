...
RemoteResponseEvent * LargeDataProxy::createRemoteRequestFailedEvent(const ProxyAddress & addrProxy, unsigned int msgId, NEService::eResultType reason, const SequenceNumber &  seqNr) const
{
...
{
    ProxyBase::ServiceAvailableEvent* event = static_cast<ProxyBase::ServiceAvailableEvent *>( DEBUG_NEW LargeDataProxy::LargeDataServiceAvailableEvent(consumer) );
    if (event != nullptr)
    {
