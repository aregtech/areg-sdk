...
areg::String areg::getDirectMessagingRole(const areg::String & nickName, uint32_t cookie, uint64_t session, bool isInitiator)
{
    areg::String result;
...
areg::String areg::getDirectConnectionName( const areg::String & nickName, uint32_t cookie, uint64_t session )
{
    areg::String result;
    if ( (nickName.isEmpty( ) == false) && (cookie != DirectConnection::InvalidCookie) )
    {
