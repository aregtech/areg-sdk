...
void WorkerThread()
{
    ...
    Thread::sleep(areg::common::WAIT_1_SECOND); // simulate some work, force the event to be signaled before wait
...
int main()
{
    VERIFY(!gEvtReady.lock(areg::common::WAIT_100_MILLISECONDS));   // should timeout, since event is non-signaled
    VERIFY(!gEvtProcess.lock(areg::common::WAIT_100_MILLISECONDS)); // should timeout, since event is non-signaled
