...
1. request a scan of a few pages with nothing else running -- expect it accepted, each
   stage and page reported, finishing with the right page count
2. while that scan is still running, request a print -- expect a refusal naming the
   reason
3. once the scan has finished, request a print of a few pages -- expect it accepted,
   each stage and page reported, finishing with the right page count
4. request a copy of a few pages -- expect the scan stages for every page, then the
   print stages for every page, in that order, and the finished page count to match
   what was asked for
5. start a new print job and cause a jam partway through it -- expect a fault naming
   the printer and how many pages it completed first, and any further job refused
   while the fault stands
6. clear the fault -- expect the device to return to idle
7. cause a jam in the **scanner**, in a fresh scan job -- expect the fault reported in
   the same shape as step 5, naming the scanner this time
8. clear the fault and run one more small print job -- expect it to succeed, proving
   the device fully recovered
