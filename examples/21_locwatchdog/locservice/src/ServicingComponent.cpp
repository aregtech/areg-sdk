...
void ServicingComponent::request_start_sleep( uint32_t timeoutSleep )
{
    ...
    // sleep() cannot be interrupted -- POSIX has no way to terminate a thread -- so the thread
    // would be abandoned and its objects leaked. wait_exit() returns the moment the restart is
    // ordered, so the thread leaves by a normal return and destroys what it owns.
