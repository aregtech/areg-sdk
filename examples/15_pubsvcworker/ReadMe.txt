...
This project demonstrates use of worker thread to perform parallel jobs and custom events for multi-threading communication.
It also demonstrates how to describe a worker thread in the model that is automatically created when the model is loaded.
...
This project is describing worker thread in the model, where worker thread is automatically created when the model is loaded.
The worker thread imitates the communication with the device (virtual device / console).
The worker thread is bound to the component and is used for the communication between the component object and the worker thread.
...
This project contains instance of public service and the worker thread, which receives data from console (virtual device)
and directly updates the attribute of the service. The service and the worker threads are described in the static model,
and they are automatically instantiated when the model is loaded.
The worker thread is bound to the component and is used for the communication between the component object and the worker thread.
