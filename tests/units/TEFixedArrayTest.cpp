...
TEST(TEFixedArrayTest, TestSearchElem)
{
    ...
    for (uint32_t i = 0; i < notEmpty.getSize(); ++i)
    {
        ...
        EXPECT_EQ(notEmpty.find(static_cast<int>(i), i + elemCount), areg::common::INVALID_INDEX);
        EXPECT_EQ(notEmpty.find(static_cast<int>(i + elemCount), 0u), areg::common::INVALID_INDEX);
    }
}
 ...
 **/
TEST(TEFixedArrayTest, TestSortAscending)
{
    using FixedArray = TEFixedArray<int>;
    ...
    FixedArray arr1(_arr1, _len), res1(_res1, _len);
 ...
 **/
TEST(TEFixedArrayTest, TestSortDescending)
{
    using FixedArray = TEFixedArray<int>;
